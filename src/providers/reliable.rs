...
mod tests {
    ...
    #[async_trait]
    impl Provider for ChatAwareMock {
        async fn chat_with_system(
            &self,
            _system_prompt: Option<&str>,
            _message: &str,
        ...
        async fn chat(
            ...
            let tools = request
                .tools
                .ok_or_else(|| anyhow::anyhow!("missing tool specs"))?;
            self.tools_seen.store(tools.len(), Ordering::SeqCst);

            Ok(ChatResponse {
                text: Some("tool-dispatched".to_string()),
        ...
        async fn chat_with_history(
            ...
            anyhow::bail!("chat_with_history should not be used for native tool requests");
        }
    ...
    #[tokio::test]
    async fn chat_forwards_tool_specs_instead_of_dropping_to_history() {
        ...
        let response = provider
            .chat(
                ChatRequest {
                    messages: &[ChatMessage::user("weather in Serbia today")],
                    tools: Some(&tools),
                },
                "test-model",
                0.2,
            ...
            .await
            .expect("chat should succeed through provider.chat");

        assert_eq!(chat_calls.load(Ordering::SeqCst), 1);
        assert_eq!(history_calls.load(Ordering::SeqCst), 0);
        assert_eq!(tools_seen.load(Ordering::SeqCst), 1);
